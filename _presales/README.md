...
เนื้อหา: อยู่ใน `build/reference.json` ทั้งหมด (ไม่เข้า git) · โครงถ้อยคำเป็น context / requirement / solution สามการ์ด ไม่ลงรายละเอียดว่าเป็นข้อไหนของแพ็กเกจ
...
**สีของหน้านี้คือสีแบรนด์ของลูกค้า** (`accent` ใน JSON ดูดจากไฟล์โลโก้) ตั้งใจให้หน้าเป็นของลูกค้ารายนั้น ไม่ใช่โทนของเราเอง — เคยทำเป็นแถบ navy แบบหน้า About Us แล้ว ดูเก่า ทิ้งไปแล้ว
โลโก้ลูกค้าวางที่ `assets/customer/` (ignored) — ได้มาเป็น .svg ให้เรนเดอร์เป็น .png ด้วย chromium ของ Playwright (`--headless --screenshot --window-size=1200,460`) เพราะ PowerPoint COM ไม่รับไฟล์ .svg · ขนาดบนสไลด์ตั้งที่ `logoWidth` ใน JSON สคริปต์ล็อกสัดส่วนให้เอง
**ก่อนเอาไปโชว์ ต้องเคลียร์กับลูกค้ารายนั้นก่อนว่าอ้างชื่อ+โลโก้ได้** หน้านี้กำกับไว้แค่ว่าเป็นเครื่องหมายการค้าของเขา
**สคริปต์เองคือข้อสอบ**: ไม่มี `reference.json` หรือไม่มีไฟล์โลโก้ = throw ไม่แตะเด็ค
