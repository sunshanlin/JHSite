...
- **Links follow the page order:** บริการ · ฟังก์ชัน · ตัวอย่างรายงาน · แพ็กเกจ · เดโม (`#billing-flow`) · FAQ · เกี่ยวกับเรา · บทความ. There is no Home link; the logo goes to the top. Between 1101 and 1240px the links tighten to 7px side padding so the bar still fits on one row in both languages
...
The working demo in `#billing-flow` is laid out as bento tiles on the `--bg-soft` ground, 12px apart, 8px corners, and no two tiles share a color. The calendar tile sits on the diagram's aqua (`--bfj-lane-a`), with its color legend inside the tile's foot, so the white day cells stand out. The customer's billing rule sits on sand (`--bfj-sand`). The billing note sits on lavender (`#F3F0FC`, border `#DCD5F3`) and stretches to line up with the calendar's bottom edge. The board has no title bar: its one top row holds the how-to line (which also reports the result of a move) with the Reset button at its right end. The three totals under the board are stat tiles in the shape of Microsoft's ROI cards: white at the top where the number sits, with a glow rising from the bottom edge in Sand, Sea, and a pale BC blue. The calendar's state colors (burgundy for late, blue for due today, jade for next cycle) never become a tile ground, so a state keeps a single meaning. Every label clears 4.5:1 on its tile's strongest stop (4.61–5.61). Below 760px the amount tile takes a full row and the other two share the row beneath it.

**The purple family.** Output that the system produces reads purple, in two shades. The billing note is bluish lavender (`#F3F0FC`). The VAT Service panel "ที่ VAT Service เติมให้" is pinkish orchid (`#F6EEF8`, with a 3px `#8E6DB5` bar that clears 3.70). The billing flow diagram uses the orchid shade for the customer lane and every loop-back path (`--bfj-lane-c`, `--bfj-orchid`, `--bfj-orchid-ink: #74509E`). That ink clears 6.21 on white, 5.54 on the aqua lane, and 5.47 on the orchid lane. Rose and red no longer appear in the diagram. Burgundy stays for things that ask for a click and for the calendar's "late" state.

### Footer

Deep Teal ground (`#01211D`), Mist and `#CBDCD8` text, one hairline rule. Three columns above 760px — brand, menu, contact — collapsing to one below. There is no copyright line; below the rule sits the cookie control alone. Column headings are `<p class="footer-head">` styled as Labels rather than real headings, so the footer does not add anything to the document outline; the menu is a `<nav>` with its own `aria-label`. Link labels are the same strings the top nav and `#contact` already use, so the footer costs the `I18N` dictionary only the two column headings.
